...
- `moveNotes`: clamps the time delta uniformly across the moved set so no moved note crosses into a non-moved same-pitch neighbour. Pitch shift clamps toward 0 from the requested |k| (largest valid shift in the requested direction wins) — single-semitone obstacles stop the move at the slot before them, but if the cursor drags PAST the obstacle (target pitch lands on a free slot beyond), the moved set jumps over. **Single-note** drags treat any same-pitch obstacle as blocking regardless of time overlap (otherwise the dragged note momentarily flashes "directly below/above" the obstacle when the cursor sweeps through its pitch column at a non-overlapping time). **Multi-select** drags use the lenient time-overlap rule — a chord/phrase transpose shouldn't freeze just because some target pitch has any other note elsewhere in the song. Touching boundaries are allowed
