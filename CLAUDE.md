...
- **Editing UX.** Pins live on a fixed 16 px strip under the ruler (`ui/SettingsPinLane.tsx`, wired in `Timeline.tsx`) — diamond markers, not a value graph. `P` / `+` adds at the playhead and a left-click on empty lane space adds at the clicked time; clicking a pin selects + seeks, drag moves, right-drag sweeps to erase every pin under the cursor, Del removes — each gesture collapses to one undo entry (begin/endSettingsEdit refcount nests with `removeKeyframe`'s own). Cursors mirror the note editor: `crosshair` on empty lane, `move` over a pin, `not-allowed` while erasing. **Edit target = the SELECTED pin, explicitly** (`editingKeyframeTime`, set by clicking a pin / `addKeyframe`; `null` = none). NOT playhead-derived — deterministic so the Inspector reliably shows what it edits. **`base` (the "default look") and each pin's snapshot are held strictly separately**: `selectKeyframe` only records the selection (it must NEVER merge the snapshot into `base` — that overwrote the default, a bug), and `updateSettings` routes by selection — a pin selected → animatable keys edit ONLY that pin's snapshot (`base` untouched); no pin selected → edit `base` (the default look the pre-first-pin region ramps from). Non-animatable keys (`isAnimatableKey` false: audio, etc.) always go to `base`. The Inspector READ side mirrors this via `useEffectiveSetting(key)` (used by every `Bound*` row + the bespoke camera / trackColors / theme reads): it shows the selected pin's snapshot value (animatable) or `base` — so a control always displays exactly what it writes. `SettingsPinLane` glows `editingKeyframeTime`; the `Inspector` banner is amber "Editing pin i/N @ m:ss" with a **Default** button (`selectKeyframe(null)`) when a pin is selected, sky "Editing default look — click a pin to edit it" when none. Del removes the selected pin. **Reset** (`resetSettings`) is split: a pin selected → reset ONLY that pin's snapshot to `pickAnimatable(defaultSettings)` (base default + other pins + everything else untouched); no pin selected → reset the `base` default look confined to what the Inspector shows, carrying over `RESET_PRESERVED_KEYS` (transport `volume`/`playbackRate`, the timeline editor's clip sync+trim = "clip length", `midiSpeedAutomation*` = "speed pins", lane layout, `followPlayhead`/`previewHighFps`, and `settingsKeyframes` — pins never wiped). **Add any new timeline/clip/session key to `RESET_PRESERVED_KEYS`** or Reset will clobber it. **Opening a different MIDI/song** (`setSong(song, { resetTimeline: true })` — passed from the Open-MIDI action, `.mid` drop, recording auto-load / pick) clears `SONG_TIED_KEYS` (pins, MIDI/audio clip offset+trim = "clip length", `midiSpeedAutomation*`) back to defaults since they're keyed to the old timeline, while keeping the Inspector look + editor layout; recorder-restore / empty-song bootstrap call `setSong` WITHOUT the flag (same song context).
