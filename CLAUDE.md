...
**Unread badge.** Each `Recording` carries a `read?: boolean`. New recordings finalise with `read: false`; a sky-blue count badge sits on the toolbar's recordings (Playlist) button while any unread exist, and each unread row in the popover gets a leading sky dot (read rows reserve the same 8px slot transparently so the layout doesn't jump). `RecorderManager.markAllRead()` flips every entry true and persists each one, called when the popover **closes** (not opens) so users can actually see which rows were new while reading the list. Hydrate backfills missing `read` to `true` so prior recordings don't surface as a backlog the first time this code ships.
...
- Window `beforeunload` prompts whenever `dirty` is true, regardless of whether a project file has been picked yet. Unnamed sessions also count — a New + edit + accidental reload loses real work, so the prompt is worth the small friction. A fresh page load with no edits leaves `dirty` false and won't prompt. Toolbar's "Unsaved" badge follows the same rule (`dirty` alone) and falls back to an `Untitled` filename label when there's no `currentFile` yet
