...
- **Editing UX.** Pins live on a fixed 16 px strip under the ruler (`ui/SettingsPinLane.tsx`, wired in `Timeline.tsx`) — diamond markers, not a value graph. `P` / `+` adds at the playhead and a left-click on empty lane space adds at the clicked time; clicking a pin selects + seeks, drag moves, right-drag sweeps to erase every pin under the cursor, Del removes — each gesture collapses to one undo entry (begin/endSettingsEdit refcount nests with `removeKeyframe`'s own). Cursors mirror the note editor: `crosshair` on empty lane, `move` over a pin, `not-allowed` while erasing. Clicking a pin only seeks to it (`selectKeyframe` records the click but **must never merge the pin snapshot into base** — base is the separate, editable "default look" the pre-first region ramps from; overwriting it was a bug). The seek alone makes the clicked pin the live target. **The edit target is purely playhead-derived** (`targetPinIndex`, exported): once any pin exists, Inspector edits / Reset / the indicator all route to the nearest pin **at or before the playhead** (the pin opening the current segment / whose look is held) — no manual selection, so the head and pin-clicks retarget automatically and there is no "deselect". If the head is before *every* pin there is **no target** (returns -1) — a future pin is never grabbed; edits land on `base` (the separate default look the pre-first region ramps from) and stay visible there. `updateSettings` mirrors the animatable patch into the target pin's snapshot so every control edits "what you see" instead of the resolver-shadowed base. The `Inspector` shows a live banner — amber "Editing pin i/N @ m:ss" when a pin is targeted, sky "Editing default look (before pin 1)" in the pre-first region — and `SettingsPinLane` glows the target pin, all recomputed from the rAF playhead via `targetPinIndex` so they track the head. **Reset is confined to what the Inspector shows.** `resetSettings` returns every Inspector-controlled key to its default but carries over `RESET_PRESERVED_KEYS` from current state — transport (`volume`/`playbackRate`), and everything the timeline editor owns: clip sync + trim ("clip length"), `midiSpeedAutomation*` ("speed pins"), lane layout, `followPlayhead`/`previewHighFps`, and `settingsKeyframes` (pins never wiped). When a pin is the edit target its *content* is additionally reset to `pickAnimatable(defaultSettings)` (other pins untouched) so Reset means "reset this pin" while pins exist. **Add any new timeline/clip/session key to `RESET_PRESERVED_KEYS`** or Reset will clobber it. Base drift is harmless: once any pin exists, animatable base values are fully shadowed by the resolver. **Opening a different MIDI/song** (`setSong(song, { resetTimeline: true })` — passed from the Open-MIDI action, `.mid` drop, recording auto-load / pick) clears `SONG_TIED_KEYS` (pins, MIDI/audio clip offset+trim = "clip length", `midiSpeedAutomation*`) back to defaults since they're keyed to the old timeline, while keeping the Inspector look + editor layout; recorder-restore / empty-song bootstrap call `setSong` WITHOUT the flag (same song context).
