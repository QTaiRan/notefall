...
**Texture animation phase is INTEGRATED, not `now() * speed`.** The liquid/gem/custom presets scroll by a `uPhase` vec2 that the `useFrame` accumulates as `phase += clamp(Δnow, 0, 0.1) * noteAnimSpeed{X,Y}`. The earlier `uTime * uAnimSpeed` form made the apparent flow velocity `animSpeed + now()·dAnimSpeed/dt`; since `now()` is a large absolute timestamp, any `noteAnimSpeed` slope between two settings-pins (even a ramp *into* two pins that hold the **same** value) exploded the flow speed. Integrating the rate makes the visual velocity exactly `animSpeed` and the phase C⁰ across pin boundaries. Phase + last-`now()` live in refs that persist across the live→export handoff (same component instance); the Δ clamp absorbs backgrounded-tab gaps, seeks, and that handoff. Driven off `now()` so export parity stays automatic. **`uPhase` is wrapped `% PHASE_WRAP` (1024)** — the same precision guard HitLine applies to its `uTime` (`% TIME_WRAP_SECONDS`): an unbounded accumulator fed into fbm/hash eventually exceeds float32-precise range, the per-frame delta drops below the ULP, and the flow visibly **freezes even at speed 1** (a long-standing bug the old `now() * animSpeed` form shared, since raw `now()` here was never wrapped). The `%` pop is one frame, tens of minutes apart, hidden in the noise churn, and deterministic in the phase so live and export wrap identically.
...
- **Data** (`midi/settingsKeyframes.ts`). `settingsKeyframes: SettingsKeyframe[]` is a key **inside `Settings`** — so it rides the existing persistence / dirty / undo / `.nfz` paths with zero new plumbing (no manifest schema bump). `SettingsKeyframe = { time /* timeline-time sec */, settings: Partial<Settings>, curvature? }`. `ANIMATABLE_KEYS` enumerates the snapshot-able continuous visual keys (camera, colours, opacity/emissive, bloom, background, …) — discrete enums/booleans, audio, timeline-layout and song-sync are excluded. **`noteAnimSpeedX/Y` are also deliberately excluded**: they are a texture-flow RATE, and keyframing a rate is ill-posed — a pin-interpolated rate makes the visible speed lurch (the resolver ramps the rate itself), which produced both the "flow explodes at the next pin" and "flow decays to a standstill over ~2 min" bugs. Excluded ⇒ `isAnimatableKey` false ⇒ the Inspector's Animation Speed always reads/writes the single base value (constant everywhere, pins irrelevant); existing pins' stale `noteAnimSpeed*` snapshot entries are simply ignored by the resolver (it only iterates `ANIMATABLE_*`). Texture POSITION still moves over time via FallingNotes' integrated phase; only its rate is global. `pickAnimatable(s)` extracts that subset.
- **Resolver** `resolveSettingsAt(base, kfs, t)` — pure. **0 pins → returns `base` by reference** (bit-for-bit identity; the whole feature is a no-op until a pin exists). `pickBlend` chooses the segment: **before the first pin → ramp from `base`** (the editable "default look", held separately from pins, acting as an implicit anchor at t=0) **into the first pin** over `[0, firstPin.time]` — this is what makes Inspector edits visible in the no-pin region (with no pin targeted, edits land on `base`); **after the last pin → hold the last pin** (that region still has a governing target pin); **between → per-type lerp** (number / `#rrggbb` via THREE.Color / vec3 / colour-map), eased by the left pin's `curvature` (same `applyCurvature` shape as `speedMap`). `resolveNoteTintAt` shares `pickBlend`, so the timeline gradient shows the same base→first-pin ramp at the head.
...
- **Editing UX.** Pins live on a fixed 16 px strip under the ruler (`ui/SettingsPinLane.tsx`, wired in `Timeline.tsx`) — diamond markers, not a value graph. `P` / `+` adds at the playhead and a left-click on empty lane space adds at the clicked time; clicking a pin selects + seeks, drag moves, right-drag sweeps to erase every pin under the cursor, Del removes — each gesture collapses to one undo entry (begin/endSettingsEdit refcount nests with `removeKeyframe`'s own). Cursors mirror the note editor: `crosshair` on empty lane, `move` over a pin, `not-allowed` while erasing. **Edit target follows the PLAYHEAD** — `editingKeyframeTime` is driven by `PinTargetSync` (single app-wide rAF in `Layout`): the nearest pin at/before the head, or `null` before the first pin / no pins. It writes the store only on a *change* (boundary crossing or seek), so the Inspector/banner/lane re-render only then, not every frame. Clicking a pin seeks to it (so it becomes the target); moving the timeline retargets — a clicked pin does NOT stick once the head moves off it. **`base` (the "default look") and each pin's snapshot are held strictly separately**: `selectKeyframe` only records the click (it must NEVER merge the snapshot into `base` — that overwrote the default, a bug), and `updateSettings` routes by `editingKeyframeTime` — a pin targeted → animatable keys edit ONLY that pin's snapshot (`base` untouched); none → edit `base` (the default look the pre-first-pin region ramps from). Non-animatable keys (`isAnimatableKey` false: audio, etc.) always go to `base`. The Inspector READ side mirrors this via `useEffectiveSetting(key)` (used by every `Bound*` row + the bespoke camera / trackColors / theme reads): it shows the targeted pin's snapshot value (animatable) or `base` — so a control always displays exactly what it writes. Pin chrome is **sky-toned to match the rest of the timeline UI** (the diamond ◇ shape — vs the speed lane's round dots — is what distinguishes the two; do NOT reintroduce a clashing accent hue). `SettingsPinLane` brightens `editingKeyframeTime` toward white; the `Inspector` banner is an understated sky/neutral status strip naming "Editing pin i/N · m:ss" when a pin is targeted, dimmed "Editing default look — before the first pin" when none. Del removes the targeted pin. **Reset** (`resetSettings`) is split: a pin targeted → reset ONLY that pin's snapshot to `pickAnimatable(defaultSettings)` (base default + other pins + everything else untouched); none → reset the `base` default look confined to what the Inspector shows, carrying over `RESET_PRESERVED_KEYS` (transport `volume`/`playbackRate`, the timeline editor's clip sync+trim = "clip length", `midiSpeedAutomation*` = "speed pins", lane layout, `followPlayhead`/`previewHighFps`, and `settingsKeyframes` — pins never wiped). **Add any new timeline/clip/session key to `RESET_PRESERVED_KEYS`** or Reset will clobber it. **Opening a different MIDI/song** (`setSong(song, { resetTimeline: true })` — passed from the Open-MIDI action, `.mid` drop, recording auto-load / pick) clears `SONG_TIED_KEYS` (pins, MIDI/audio clip offset+trim = "clip length", `midiSpeedAutomation*`) back to defaults since they're keyed to the old timeline, while keeping the Inspector look + editor layout; recorder-restore / empty-song bootstrap call `setSong` WITHOUT the flag (same song context). **Dragging the MIDI clip** (`Timeline.tsx` `onMidiPointerMove` → `store.shiftKeyframes`) translates every pin's `time` by the exact applied `midiOffset` delta so the colour automation stays glued to the notes — `midiOffset` is a pure additive shift of every note's TL_audio time, so the same delta on each pin keeps them locked regardless of the speed map; trimming does NOT shift pins (trim only hides, it doesn't reposition notes). `shiftKeyframes` is a plain set (no internal history) — the clip drag's own begin/endSettingsEdit bracket collapses the whole move to one undo entry; it runs BEFORE the `updateSettings({ midiOffsetSec })` set so the latter reads (and preserves) the already-shifted `settingsKeyframes`.
...
Module-level `Clock` with `setActiveClock` / `resetActiveClock` / `now()`. Default is the real wall clock (`performance.now() / 1000`). Engine + visual effects (HitParticles, LandingFlashes, HitLine, FallingNotes' `uPhase` integrator, customTexture animator) all read `now()` instead of `performance.now()` so the exporter can swap in a `VirtualClock` and step them at non-realtime intervals.
