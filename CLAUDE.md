...
**Auto-keyframe** (DAW-style). When pins exist but none is explicitly selected, an animatable Inspector edit would otherwise be invisible (resolver shadows base). So `updateSettings` writes it at the playhead instead: update the pin under the head (±ε) or drop a new one capturing the resolved look (curve doesn't jump), then stamp + auto-select it. The whole drag stays one undo entry (pin list is in `settings`; the gesture baseline predates the new pin). **Crucial guard:** this only fires when `pendingSettingsDepth > 0` — i.e. inside an Inspector begin/endSettingsEdit gesture. Camera orbit/dolly (`CameraControls`) and transport side-effects call `updateSettings` *outside* a gesture, so navigation never spawns pins; only deliberate Inspector edits do.
